    ...
    boxShadow: [BoxShadow(color : Colors.black87, blurRadius : blur, offset : Offset(offset, offset))]
        ...
        onPressed:() {
          ...
          Navigator.push(context,
              MaterialPageRoute<void>(builder: (BuildContext context) => Detail(data : data))
        ...
        child: Column( // 아래
          children: <Widget>[
            Row( // 옆으
              children: <Widget>[
                Card(
                  child : Image.network(data['img'],width : 200 , height : 280),
                ...
                Column(
                  children: <Widget>[
                    Text(data['title'], style:TextStyle(fontSize: 30, fontWeight:FontWeight.bold, color: Colors.grey[900])),
                    Text(data['casting'][0]),
                  ],
                )
            ...
            Row(
              children: <Widget>[
                Text("개요  ", style:TextStyle(fontSize: 20, fontWeight:FontWeight.bold, color: Colors.grey[700])),
                for(var i in data['tags'])
                Text('  ' + i, style:TextStyle(fontSize: 20, fontWeight:FontWeight.bold, color: Colors.blue[900]))
              ],
            ),
            Row(
              children: <Widget>[
                Text("출연  ", style:TextStyle(fontSize: 20, fontWeight:FontWeight.bold, color: Colors.grey[700])),
                Column(
                    children: <Widget>[
                      for(var i in data['casting'])
                        Text('  ' + i, style:TextStyle(fontSize: 20, fontWeight:FontWeight.bold, color: Colors.blue[900])),
                    ],
                ),
              ],
